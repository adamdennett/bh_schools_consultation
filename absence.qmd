...
-   As a consequence of misdiagnosing the problem, the treatment prescribed by the council not only fails to address the real underlying issue, but it actually stands to make it much worse. The 20% out-of-catchment open admissions criteria, if implemented, will lead to up to 30% of the City's children having to attend a school out of catchment (20% through 'choice' and 10% through forced displacement). Forced long-distance travel and the disconnection between local community, family and school is guaranteed to exacerbate the attendance problem and thus drive down attainment in the city (which currently is above average).
...
-   Multiple regression is particularly useful as including a number of independent variables at the same time in a model allows to their effects on the dependent variable to be evaluated at the same time. This is particularly useful when there might be some degree of correlation between independent variables such as in our case with persistent absence and disadvantage.
...
-   In the first year, the total number of displaced students travelling long distances will be 677 - according to the figures released by the council (student estimates, multiplied by published probabilities).
...
-   I will not reiterate the environmental, social and community impacts of this as they have been voiced many times and they alone should be enough to make the council pause. However, here I am just focusing on educational outcomes, so lets look at those.
