...
lm_model <- lm(ATT8SCR ~ PPERSABS10, data = england_school_2022_23_not_special)
lm_summary <- summary(lm_model)
lm_coefficients <- lm_summary$coefficients
intercept <- round(lm_coefficients[1, 1], 2)
slope <- round(lm_coefficients[2, 1], 2)
...
lm_model <- lm(ATT8SCR ~ PTFSM6CLA1A, data = england_school_2022_23_not_special)
lm_summary <- summary(lm_model)
lm_coefficients <- lm_summary$coefficients
intercept <- round(lm_coefficients[1, 1], 2)
slope <- round(lm_coefficients[2, 1], 2)
...
ggplot(england_school_2022_23_not_special, aes(y = ATT8SCR, x = PTFSM6CLA1A)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +  # Add linear model line for england_school_2022_23
  geom_smooth(method = "lm", formula=(y ~ exp(-0.09*x)), se = TRUE, colour = "red") + geom_smooth(method = "loess", formula=(y ~ x), se = TRUE, colour = "green") + 
...
\*note need to figure out the function here - not linear
...
# Base plot with england_school_2022_23
plot <- ggplot(filtered_data, aes(y = ATT8SCR, x = PPERSABS10, colour = OFSTEDRATING)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  # Add linear model line for england_school_2022_23
  labs(title = "Attainment 8 vs Persistent Absence %, 2022-23",
       x = "% Enrollments who are persistently absent",
...
ggplot(filtered_data, aes(y = ATT8SCR, x = PPERSABS10, colour = OFSTEDRATING)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  # Add linear model line for england_school_2022_23
  labs(title = "Attainment 8 vs Persistent Absence %, 2022-23",
       x = "% Enrollments who are persistently absent",
...
geom_point(data = btn_sub, aes(y = ATT8SCR, x = PPERSABS10, colour = OFSTEDRATING)) +
  geom_smooth(data = btn_sub, aes(y = ATT8SCR, x = PPERSABS10), method = "lm", se = TRUE, color = "black") +  # Add linear model line for btn_sub
  geom_text_repel(data = btn_sub, aes(y = ATT8SCR, x = PPERSABS10, label = SCHNAME.x), color = "black", size = 3, nudge_y = c(-1.5, 1.5), force = 10, box.padding = 0.5, max.overlaps = 10, direction = "both") +  # Add linear model line for england_school_2022_23
  labs(title = "Attainment 8 vs Persistent Absence %, 2022-23",
       x = "% Enrollments who are persistently absent",
       y = "Attainment 8 measure after adjustment for extreme scores") +
  theme_minimal()
...
plot <- ggplot(filtered_data, aes(y = ATT8SCR, x = PTFSM6CLA1A, colour = OFSTEDRATING)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  # Add linear model line for england_school_2022_23
...
# Add another layer with btn_sub points and labels with sticks
plot + 
  geom_point(data = btn_sub, aes(y = ATT8SCR, x = PTFSM6CLA1A), color = "black") +
  geom_smooth(data = btn_sub, aes(y = ATT8SCR, x = PTFSM6CLA1A), method = "lm", se = FALSE, color = "black") +  # Add linear model line for btn_sub
  geom_text_repel(data = btn_sub, aes(y = ATT8SCR, x = PTFSM6CLA1A, label = SCHNAME.x), color = "black", size = 3, nudge_y = c(-1.5, 1.5), force = 10, box.padding = 0.5, max.overlaps = 10, direction = "both")
...
```{r echo=T, message=FALSE, warning=FALSE}
#| label: fig-plot35
#| fig-cap: "Attainment 8 vs % Disadvantaged, by OfSted Rating, All Schools in England, 2022/23."

ggplot(filtered_data, aes(y = ATT8SCR, x = PTFSM6CLA1A, colour = OFSTEDRATING)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  # Add linear model line for filtered_data
  labs(title = "Attainment 8 vs Disadvantaged Student %, 2022-23",
       x = "% Percentage of Disadvantaged Students",
...
The fact that the scatter plot in @fig-plot14 seems to suggest there is a weak relationship between disadvantage and attainment in Brighton should be ringing big alarm bells for Brighton and Hove Council at this point. The consultation is built upon the premise that poor attainment in the city is driven by disadvantage, when the evidence presented here is suggesting at best a weak association and one which is dwarfed by the persistent absence problem.
