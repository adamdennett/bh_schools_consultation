...
# Read the GeoPackage
BN_pcds_to_school_dist <- st_read("data/BN_pcds_to_school_dist.gpkg") %>% 
  st_transform(4326)

# Assuming bn_pcds_r5r is already loaded with 'lon' and 'lat' columns

subset_data <- BN_pcds_to_school_dist %>% filter(total_cost < 4828.03)

#Longhill 114581

longhill_bus_free <- subset_data %>% filter(destination_id == 114581)
...
map <- leaflet() %>% 
  ...
  addPolylines(data = longhill_bus_free, 
               color = "blue", 
               weight = 2, 
               opacity = 0.5) %>%  # Display "id" on mouse-over
  addCircleMarkers(data = bh_sec_sch1, 
                   ~long, ~lat, 
                   color = "black", 
                   radius = 4,  # Increase radius for prominence
