...
Travel Times to stringer at 7:30 am on 1/12/22 from BN Postcodes
...
ttm_brighton_pcds_2_stringer = travel_time_matrix(r5r_core = r5r_core,
                          origins = bn_pcds_r5r,
...
pcds_2_stringer = aggregate(ttm_brighton_pcds_2_stringer$travel_time_p50, by=list(ttm_brighton_pcds_2_stringer$from_id), FUN=min, na.rm=TRUE)

pcds_2_stringer <- rename(pcds_2_stringer, id = Group.1) 
pcds_2_stringer <- rename(pcds_2_stringer, time = x)

pcds_geo_stringer = inner_join(bn_pcds_r5r, pcds_2_stringer, by="id") %>% 
  st_as_sf(coords = c("lon", "lat")) %>% 
  st_set_crs(4326)

pcds_geo_stringer$time <- as.numeric(pcds_geo_stringer$time)
pcds_geo_stringer$color <- ifelse(pcds_geo_stringer$time >= 75, "black", NA)

dbWriteTable(con, name="pcds_geo_stringer_walk_bus", value=pcds_geo_stringer, overwrite=TRUE)

tmap_mode("view")
tm_shape(pcds_geo_stringer) + 
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
# set inputs
origins = bn_pcds_r5r %>% filter(id == "BN2 5QN") 
destinations = bh_sec_sch1
